...
7. **CTA種別の選定**(2026-07-29改訂: CTAは1つに絞る方が反応が出やすいというユーザー判断により、
   塾系記事の5択選定は廃止): 記事のカテゴリで決まる。
   - **習い事紹介**(手順1でnaraigoto-*の季節テーマを採用した場合): そのテーマの `allowed_cta` に
     含まれるブランド別CTA(`soroban_trial`/`eikaiwa_trial`等)を使う(従来通り)。
   - **それ以外(塾系記事、地域情報/勉強のコツ/入試情報/保護者コラムの全て)**: 季節テーマの
     `allowed_cta` の内容に関わらず、常に `free_consultation`(無料学習相談)を `cta_type` に
     記録する。個別テーマに応じた使い分けは行わない。
