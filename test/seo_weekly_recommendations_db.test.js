...
test('UNIQUE制約: 同一batch_dateの2件目INSERTはDBレベルでエラーになる', () => {
  ...
  const insert = () =>
    conn
      .prepare(
        `INSERT INTO seo_weekly_recommendations (batch_date, status, task_ids, items, created_at, updated_at)
         VALUES ('2026-07-13', 'proposed', '[]', '[]', '2026-01-01T00:00:00Z', '2026-01-01T00:00:00Z')`
      )
      .run();
