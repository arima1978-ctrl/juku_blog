...
const TMP_DISABLED_CONFIG = path.join(os.tmpdir(), `juku_blog_weekly_director_disabled_config_${process.pid}.yaml`);
...
after(() => {
  closeDb();
  [TMP_DB, TMP_CONFIG, TMP_DISABLED_CONFIG].forEach((f) => {
    try {
      fs.unlinkSync(f);
    } catch {
      // 既に無ければ無視
...
test('CLI: growth_director.enabled=false(既定)なら無処理で終了する', () => {
  const output = execFileSync('node', [path.join(ROOT, 'scripts', 'seo_weekly_director.js'), '--dry-run'], { cwd: ROOT, encoding: 'utf8', env: disabledEnv });
