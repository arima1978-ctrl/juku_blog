...
function seedApprovedRecommendation(batchDate, items) {
  ...
  seoDb.transitionWeeklyRecommendationStatus({ batchDate, expectedCurrentStatus: 'proposed', nextStatus: 'approved' }, nowIso);
}
...
test('正常系: approvedな週次推薦の未処理create_article Taskがpublished_draftへ遷移しwp_post_idが記録される', async () => {
  ...
  const rec = seoDb.getWeeklyRecommendation(batchDate);
