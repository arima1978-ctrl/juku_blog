...
test('結合: ダッシュボードからの編集(updatePostFields)がtitle/body_md等を更新できる', () => {
  const id = makePost({ title: '編集前タイトル' });
  const changes = updatePostFields(id, { title: '編集後タイトル', body_md: '編集後本文', body_html: '<p>編集後本文</p>' });
  assert.equal(changes, 1);
  const post = getPostById(id);
  assert.equal(post.title, '編集後タイトル');
  assert.equal(post.body_md, '編集後本文');
});
