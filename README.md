...
`data/posts.sqlite` の日次自動バックアップ(`scripts/backup_db.sh`、`data/backups/`配下に直近7世代を保持)
も、記事生成の少し前に登録する(2026-07-17のデータ消失インシデントを受けて追加):

```cron
45 4 * * * cd /path/to/juku_blog && ./scripts/backup_db.sh >> logs/backup.log 2>&1
```

競合キーワード分析(Keyword Gap Lite)の週次バッチ(`scripts/seo_weekly_analysis.sh`、競合クロール→
ページ解析→Gap計算を全校舎分実行)は、日次バックアップ(04:45)・記事生成(05:00)と時間帯が
被らず、かつ実行結果が同日朝のバックアップに含まれる日曜01:00に登録する
(`features.competitor_keyword_analysis.enabled`/`crawl_enabled`が両方trueの場合のみ動作。
2026-07-17、両校舎(小幡校7件・あま本部校6件)で疎通確認済み):

```cron
0 1 * * 0 cd /path/to/juku_blog && ./scripts/seo_weekly_analysis.sh >> logs/seo_weekly.log 2>&1
```
